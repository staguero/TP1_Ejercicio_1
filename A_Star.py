class A_Star():
    ...
    def check_list(self,i,j):
        if self.mapa[i][j].id is not None:
            if self.mapa[i][j].id[0:5] == "Shelf":
                if self.mapa[i][j].id == self.target_node.id: #debo agregar el objetivo a lista abierta
                    if (self.mapa[i][j].parent == self.current_node): #si el padre es el nodo inicial no lo guardo
                        return
                else:
                    return
    ...
    def update_parent(self,old_parent,new_candidate):
        if old_parent.parent is not None:
            if old_parent.id==self.target_node.id:          #Modificación nodo padre del objetivo se actualice si el actual es el nodo inicial
                if old_parent.parent==self.current_node:    #aunque la distancia del nuevo padre sea mayor (obvio si están juntos los casilleros)
                    return True 
        ...
        try:
            ...
            for node in self.open_list:
                if (node.G + node.H) <= (minF.G + minF.H):
                    if node.id==self.target_node.id:
                        if (self.target_node.parent!=self.current_node):    #modificación por lo mismo
                            minF = node                                     #si el mínimo es el actual pero su nodo es el padre, no lo uses
                    else:
                        minF = node
            return minF
        except:
