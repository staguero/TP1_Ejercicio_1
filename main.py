...
if __name__ == "__main__":
    ...
    for j in m:
        for i in m2:
            ...
            for k in range(columnas_estante): 
                mapa[int(i)][col].id = "Shelf" + str(valor)
                mapa[int(i)+1][col].id = "Shelf" + str(valor+1)
                col+=1
                valor+=1
    ...
    close_list=[]
    path = []
    a_estrella=Node(0,0,None)
    path = a_estrella.camino(current_node,target_node,close_list,open_list,mapa,mapa_columnas,mapa_filas)
    print(path)
