...
if __name__ == "__main__":
    ...
    for j in range(mapa_columnas):
        for i in range(mapa_filas):
            if mapa[i][j].setID == "Empty":
                mapa_dibujo[i][j]='0'
            else:
                mapa_dibujo[i][j]=mapa[i][j].setID[5:]
    for i in range(mapa_filas):
        print(mapa_dibujo[i])
    ...
    for j in range(mapa_columnas):
        for i in range(mapa_filas):
            if mapa[i][j].setID[5:]==inicio:
                current_node = mapa[i][j-1]
            if mapa[i][j].setID[5:]==fin:
                target_node = mapa[i][j+1]

    open_list=[]
    close_list=[]
    path = []
    a_estrella=A_Start()
    path = a_estrella.start_method(current_node,target_node,close_list,open_list,mapa,mapa_columnas,mapa_filas)
    print(path)